...
def initialize_services():
    ...
    firebase_app = initialize_app(cred, {'storageBucket': config.FIREBASE_STORAGE_BUCKET})
    ...
    return firebase_manager, similar_service
...
async def async_main():
    ...
    firebase_manager, similar_service = initialize_services()
    ...
    try:
        ...
        similar_rooms = await similar_service.generate_img2img_from_pinterest(
            room=room,
            pinterest_url=public_url
        )
        similar_rooms = await similar_service.generate_text2img_from_pinterest(
            room=room
        )
